...
- Please try again in a few moments`;
  ...
  async transcribeAudio(audioFile: File): Promise<VoiceTranscriptionResult> {
    try {
      const formData = new FormData();
      formData.append("file", audioFile);
      formData.append("model", "gpt-4o-transcribe"); // Use the standard Whisper model for better accuracy
      formData.append("response_format", "json"); // Get more detailed response with confidence
      ...
      formData.append(
        "prompt",
        "This is a conversation about a child's behavior, ADHD, autism, school, daily activities, emotions, and family interactions. The speaker may mention specific behaviors, challenges, or observations about their child."
      );
